...
export function Contact() {
    return (
        <Section id="contact" maxWidth="full" className="px-4 md:px-20">
            <div className="bg-primary text-white rounded-[3rem] px-8 md:px-20 py-20 overflow-hidden relative shadow-2xl shadow-primary/30">
                {/* Blob Decoration */}
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="space-y-6 pt-4">
                            <a href={`mailto:${content.personalInfo.email}`} className="flex items-center gap-4 group">
                                ...
                                <span className="font-bold text-lg">{content.personalInfo.email}</span>
                            </a>
                            {content.personalInfo.phone && (
                                <a href={`tel:${content.personalInfo.phone}`} className="flex items-center gap-4 group">
                                    ...
                                    <span className="font-bold text-lg">{content.personalInfo.phone}</span>
                                </a>
                            )}
                            <a href="#" className="flex items-center gap-4 group">
                                ...
                                <span className="font-bold text-lg">{content.personalInfo.instagram}</span>
                            </a>
                        </div>
                    </div>

                    <div className="bg-white rounded-[2.5rem] p-10 shadow-3xl text-foreground">
                        <form className="space-y-6">
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-xs font-black uppercase tracking-widest text-primary">Nombre</label>
                                    <input
                                        className="w-full h-12 rounded-xl bg-primary/5 border-none focus:ring-2 focus:ring-primary px-4 font-medium"
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-black uppercase tracking-widest text-primary">Asunto</label>
                                <input
                                    className="w-full h-12 rounded-xl bg-primary/5 border-none focus:ring-2 focus:ring-primary px-4 font-medium"
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-black uppercase tracking-widest text-primary">Mensaje</label>
                                <textarea
                                    className="w-full rounded-xl bg-primary/5 border-none focus:ring-2 focus:ring-primary p-4 font-medium"
                                    placeholder="Cuéntame sobre tu propuesta..."
                                    rows={4}
                                />
                            </div>
                            <Button className="w-full h-16 rounded-2xl text-lg gap-3">
